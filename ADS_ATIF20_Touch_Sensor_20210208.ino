...
#define ATIF20_ID_GND  0x34 //0x68>>1 (0b0110100 7bit + W/R 1 bit, ID Pin=GND)
#define ATIF20_ID_VDD 0x2C  //0x58>>1 (0b0101100 7bit + W/R 1 bit, ID Pin=VDD)
// ===================================================================//
...
void Register_Dump()
{
   ...
   for (int i = 0; i < 256; i += 16)
   {
      for (int j = 0; j <= 15; j++)
      {
         ...
         Wire.requestFrom(ATIF20_ID_VDD, 1);  // data read (2 byte)
...
// ---------------- user Register ---------------------//   
   ...
   //Wire.write(0x66); // 0x66 (reset value)
   Wire.write(0x67); // 0x67 (down cal. speed -> slow)
   Wire.endTransmission(); //   
...
//------------------ Software Reset Disable (Clear) ---------------------
   Wire.beginTransmission(ATIF20_ID_VDD); // 
   Wire.write(byte(Global_ctrl1)); // 0x2Ch
   //Wire.write(byte(0x48)); // Response off : 3, Respose on : 3, Normal Mode
   Wire.write(byte(0x2A)); // Response off : 2, Respose on : 3, Fast Mode
   //Wire.write(byte(0x26)); // Response off : 2, Respose on : 2, Fast Mode
   Wire.endTransmission(); //
